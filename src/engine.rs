...
impl<'a> Engine<'a> {
    ...
    pub fn new(program: Program) -> Self {
        ...
        engine.mem_write(program.start(), program.data()).unwrap();

        engine
            .add_code_hook(program.start(), 0, |emu, addr, len| {
                println!("code exec: {addr:x}, {len}");
        ...
        engine
            .add_intr_hook(|emu, intr_number| {
                println!("got interrupted! {intr_number} {intr_number:x}");
                emu.emu_stop().unwrap();
            })
            .unwrap();
